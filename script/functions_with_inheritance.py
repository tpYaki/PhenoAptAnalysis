...
def split_hpo_id(cell):
    ...
    if cell[0] == '[':
        ...
        if ";" in cell:
            hpo_id_input = [k for k in cell.split(";")]
        else:
            if "," in cell:
                hpo_id_input = [k for k in cell.split(",")]
    if ',' in str(hpo_id_input):
        return hpo_id_input
    else:
        return [hpo_id_input]

def output_columns(hpo, tools, intersect, REVEL_thresh, CADD_thresh, collect_variants_tofile, collect_organ,change_weight=False):
    columns_fill = ['CaseID', 'hpo', 'hpo_id_input', 'Symbol']
    phenoapt_mode = ['all_hpo_plus_weight', 'only_weight_hpo_weight', 'only_weight_hpo_no_weight']
    if change_weight:
        for k in range(5):
            phenoapt_mode = phenoapt_mode + [f'Weight_weight_{k+1}']
    for tool in tools:
...
def process_rank_result_table(tool, result_mode, result_list, case_id, i, pathogenic_gene_dict, pwd, filename,
                              intersect, REVEL_thresh, CADD_thresh, part_table, part_rr_table, part_rp_table):
    txt_dir_dict = {'Symbol': 'candidategene', 'Ensembl Gene ID': 'candidategene_ensemblid',
                    'HGNC': 'candidategene_hgncid'}
    ...
    if intersect:
        ...
        if tool == 'phenomizer':
            ...
            part_table.loc[i, f'{tool}_rank_intersect'], part_rr_table.loc[i, f'{tool}_rank_intersect'], \
            part_rp_table.loc[i, f'{tool}_rank_intersect'] = get_rank(pathogenic_gene_dict[result_mode],
                                                                      tool_gene_intersect_list)
    ## Pathogenic score模式统计
    if len(REVEL_thresh) != 0:
        tsv_candidate_gene_revel_dict = {}
        for thresh in REVEL_thresh:
            tsv_candidate_gene_revel_dict[thresh] = load_pdtxt_list(
                f'{pwd}/{filename}/revel_{thresh}{Filter_txt_dir_dict[result_mode]}/{case_id}.txt')
        for thresh in REVEL_thresh:
            if tool == 'phenomizer':
                ...
                part_table.loc[i, f'{tool}_rank_revel_{thresh}'], part_rr_table.loc[
                    ...
                    i, f'{tool}_rank_revel_{thresh}'] = get_phenomizer_rank(pathogenic_gene_dict[result_mode],
                                                                            tool_gene_revel_list)
            else:
                ...
                part_table.loc[i, f'{tool}_rank_revel_{thresh}'], part_rr_table.loc[
                    i, f'{tool}_rank_revel_{thresh}'], part_rp_table.loc[
                    i, f'{tool}_rank_revel_{thresh}'] = get_rank(pathogenic_gene_dict[result_mode],
                                                                 tool_gene_revel_list)
    if len(CADD_thresh) != 0:
        tsv_candidate_gene_cadd_dict = {}
        for thresh in CADD_thresh:
            tsv_candidate_gene_cadd_dict[thresh] = load_pdtxt_list(
                f'{pwd}/{filename}/cadd_{thresh}{Filter_txt_dir_dict[result_mode]}/{case_id}.txt')
        for thresh in CADD_thresh:
            if tool == 'phenomizer':
                tool_gene_cadd_list = []
                for y in result_list:
                ...
                part_table.loc[i, f'{tool}_rank_cadd_{thresh}'], part_rr_table.loc[
                    ...
                    i, f'{tool}_rank_cadd_{thresh}'] = get_phenomizer_rank(pathogenic_gene_dict[result_mode],
                                                                           tool_gene_cadd_list)
            else:
...
def processdict(file_dir_dict, mode, hpo, case_id, inheritance):
    ...
    file_dir_dict['Xrare'][
        mode] = f'{hpo}_Xrareoutput{mode}/{case_id}.tsv'


def statistic_to_table(df, pwd, filename, hpo, tools, intersect, REVEL_thresh, CADD_thresh, collect_variants_tofile,
                       collect_organ,change_weight=False,integrated_tools=False):
    columns_fill = output_columns(hpo, tools, intersect, REVEL_thresh, CADD_thresh, collect_variants_tofile,
                                  collect_organ,change_weight)
    ...
    ##for R是后面去掉NA的。
    if hpo=='Weight' and 'phenoapt' in tools:
        phenoapt_mode = ['all_hpo_plus_weight', 'only_weight_hpo_weight', 'only_weight_hpo_no_weight']
        if change_weight:
            for k in range(5):
                phenoapt_mode = phenoapt_mode + [f'Weight_weight_{k + 1}']
    ...
    for i in tqdm(range(len(df))):
        ...
        try:
            hpo_id_input = split_hpo_id(df.loc[i,hpo])
        ...
        pathogenic_gene_dict['hgnc_id'] = 'HGNC:' + str(int(pathogenic_gene_dict['HGNC']))
        ...
        part_table.loc[i, 'CaseID'], part_rr_table.loc[i, 'CaseID'], part_rp_table.loc[
            i, 'CaseID'] = case_id, case_id, case_id
        part_table.loc[i, 'hpo'], part_rr_table.loc[i, 'hpo'], part_rp_table.loc[i, 'hpo'] = hpo, hpo, hpo
        part_table.loc[i, 'hpo_id_input'], part_rr_table.loc[i, 'hpo_id_input'], part_rp_table.loc[
            i, 'hpo_id_input'] = str(hpo_id_input), str(hpo_id_input), str(hpo_id_input)
        part_table.loc[i, 'Symbol'], part_rr_table.loc[i, 'Symbol'], part_rp_table.loc[
        ...
        mode_dict = {}
        mode_dict = {'result': {'PhenoGenius': 'Symbol', 'phenolyzer': 'Symbol',
                                'GADO': 'Ensembl Gene ID', 'phenoapt': 'Symbol', 'HANRD': 'HGNC',
                                'PhenoRank': 'Ensembl Gene ID',
                                'phenomizer': 'Symbol'},
                     'dir': {'PhenoGenius': f'{hpo}_PhenoGenius_output/{case_id}.tsv',
                             'phenolyzer': f'phenolyzeroutput/{case_id}_{hpo}.final_gene_list',
                             'GADO': f'{hpo}_GADOoutput/{case_id}.txt',
                             'phenoapt': f'phenoaptoutput/{case_id}_hpo_id_phenoapt_rank.tsv',
                     ...
                     'column': {'PhenoGenius': 'gene_symbol', 'phenolyzer': 'Gene',
        ...
        if hpo == 'Weight' and 'phenoapt' in tools:
            def get_phenoapt_result_dir(k):
                ...
                if k == 'only_weight_hpo_weight':
                    ...
                    return pheno_result_dir
                if k in [f'Weight_weight_{weight+1}' for weight in range(5)]:
                    pheno_result_dir = f'phenoaptoutput/{case_id}_{k}_phenoapt_rank.tsv'
                    return pheno_result_dir
        ...
        for tool in tools:
            if tool in mode_dict['result'].keys():
                try:
                    ...
                    part_table, part_rr_table, part_rp_table = process_rank_result_table(tool,
                                                                                         mode_dict['result'][tool],
                                                                                         tool_gene_list, case_id, i,
                                                                                         pathogenic_gene_dict, pwd,
                                                                                         filename,
                                                                                         intersect, REVEL_thresh,
                                                                                         CADD_thresh, part_table,
                                                                                         part_rr_table, part_rp_table)
                    print(f'{tool} ok')
                except Exception as e:
                    log.loc[i, tool] = e
        ...
        file_dir_dict = {
            ...
            'LIRICAL': {'original': f'{hpo}_LIRICALoutput/{case_id}.tsv',
                        'column': 'entrezGeneId',
                        'result': 'entrezGeneId'},
            'Xrare':{'original': f'Xrareoutput/{case_id}_{hpo}.csv',
                     'column': 'hgnc_id',
                     'result': 'hgnc_id'},
            'phrank':{'original':f'phrankoutput/{case_id}_{hpo}_nogenelist_phrank_rank.tsv',
                      'column':'1',
                      'result':'Ensembl Gene ID'}

        }
        if intersect:
            file_dir_dict['phen2gene']['intersect'] = f'phen2geneoutput/{case_id}-phen2gene-candidategene-output.txt'
            file_dir_dict['phrank']['intersect'] = f'phrankoutput/{case_id}_{hpo}_candidategene_ensemblid_phrank_rank.tsv'
            processdict(file_dir_dict, 'intersect', hpo, case_id, inheritance)
        if len(REVEL_thresh) != 0:
            for thresh in REVEL_thresh:
                file_dir_dict['phen2gene'][
                    f'revel_{thresh}'] = f'phen2geneoutput/{case_id}-phen2gene-revel_{thresh}-output.txt'

                file_dir_dict['phrank'][
                    f'revel_{thresh}'] = f'phrankoutput/{case_id}_{hpo}_revel_{thresh}_ensemblid_phrank_rank.tsv'
        ...
        if len(CADD_thresh) != 0:
            for thresh in CADD_thresh:
                ...
                file_dir_dict['phrank'][
                    f'cadd_{thresh}'] = f'phrankoutput/{case_id}_{hpo}_cadd_{thresh}_ensemblid_phrank_rank.tsv'
                ...
                for mode in file_dir_dict[list(file_dir_dict.keys())[0]].keys():
                    if mode not in ['column', 'result']:
                        try:
                            ...
                            if mode == 'original':
                                part_table.loc[i, f'{tool}_rank'], part_rr_table.loc[i, f'{tool}_rank'], \
                                part_rp_table.loc[
                                    i, f'{tool}_rank'] = get_rank(pathogenic_gene_dict[file_dir_dict[tool]['result']],
                                                                  tool_gene_list)
                            else:
                                part_table.loc[i, f'{tool}_rank_{mode}'], part_rr_table.loc[i, f'{tool}_rank_{mode}'], \
                                part_rp_table.loc[
                                    i, f'{tool}_rank_{mode}'] = get_rank(
                                    pathogenic_gene_dict[file_dir_dict[tool]['result']], tool_gene_list)
                            print(f'{tool} ok')
                        except Exception as e:
        ...
        part_table.to_csv(f'{pwd}/{filename}/{hpo}_{len(tools)}_rank_len_{len(df)}.tsv', sep='\t')
        part_rr_table.to_csv(f'{pwd}/{filename}/{hpo}_{len(tools)}_rr_len_{len(df)}.tsv', sep='\t')
        part_rp_table.to_csv(f'{pwd}/{filename}/{hpo}_{len(tools)}_rp_len_{len(df)}.tsv', sep='\t')
        part_table_for_R.to_csv(f'{pwd}/{filename}/{hpo}_{len(tools)}_rank_with_vcf_len_{len(df)}.tsv', sep='\t')
        part_rr_table_for_R.to_csv(f'{pwd}/{filename}/{hpo}_{len(tools)}_rr_with_vcf_len_{len(df)}.tsv', sep='\t')
        part_rp_table_for_R.to_csv(f'{pwd}/{filename}/{hpo}_{len(tools)}_rp_with_vcf_len_{len(df)}.tsv', sep='\t')
...
def transform_HANRD_from_server(df, pwd, filename, hpo):
    # ## 调整文件header
    for i in range(len(df)):
        case_id = df.loc[i, 'Blood ID']
        if f'{case_id}_{hpo}.tsv' not in os.listdir(f'{pwd}/{filename}/HANRDoutput/output/'):
            print(f'{case_id} result not ready')
        else:
            HANRD_seperate_df = pd.read_csv(f'{pwd}/{filename}/HANRDoutput/output/{case_id}_{hpo}.tsv',
                                            header=None,
                                            sep='\t')
            if  HANRD_seperate_df.iloc[0,0] != 'id':
                HANRD_seperate_df.columns = ['id', 'rank', 'HGNCid', 'score', '-', 'gene_note', 'chr', 'note']
                HANRD_seperate_df.to_csv(f'{pwd}/{filename}/HANRDoutput/output/{case_id}_{hpo}.tsv',index=False,
                                            sep='\t')
                print(f'{case_id} result transformed')
            else:
                print(f"{case_id} result's formation is ok")


def get_HANRD(df, pwd, filename, hpo, refresh=False):
    ##一个一个跑
    # print('start')
    ...
    for i in range(len(df)):
        ...
        for k in hpo_id_input:
            HPO = HPO + str(k) + ', '
        print(HPO[:-2])
        HANRD_input.loc[0, 'HPO'] = HPO[:-2]
        HANRD_input.loc[0, 'ID'] = df.loc[i, 'Blood ID']
        HANRD_input.loc[0, 'HGNC (Casual Gene)'] = '-'
        HANRD_input.loc[0, 'Orphanet ID (Diagnosed Disorder)'] = '-'
        ...
        if f'{case_id}_{hpo}.tsv' not in os.listdir(f'{pwd}/{filename}/HANRDoutput/output') or refresh:
            ...
            try:
                ...
                os.system(
                    f'mv {pwd}/{filename}/HANRDoutput/{case_id}_{hpo}_gene.out {pwd}/{filename}/HANRDoutput/output/{case_id}_{hpo}.tsv')
...
def get_PhenoRank(df, pwd, filename, hpo, refresh=False):
    ...
    log = pd.DataFrame(columns=['id','hpo_id','bug'])
    for i in range(len(df)):
        if 'HP:' in str(df.loc[i, f'{hpo}_PhenoRank']):
            hpo_reset = f'{hpo}_PhenoRank'
            hpo_id_input = split_hpo_id(df.loc[i, hpo_reset])
        else:
            hpo_id_input = split_hpo_id(df.loc[i, hpo])
        ...
        for k in hpo_id_input:
            HPO = HPO + str(k) + ';'
        case_id = df.loc[i, 'Blood ID']
        log.loc[i,'id'] = case_id
        log.loc[i,'hpo_id'] = HPO
        ...
        if f'{case_id}_{hpo}.tsv' not in os.listdir(f'{pwd}/{filename}/PhenoRankoutput') or refresh:
            ...
            print(cmd)
            try:
                f=subprocess.getstatusoutput(cmd)
                log.loc[i, 'bug'] = f[1]
            except Exception as e:
                log.loc[i,'bug'] = e
        ...
        log.to_csv(f'{pwd}/{filename}/log/PhenoRank.csv')
...
def GADO(df, pwd, filename, hpo, refresh=False):
    ...
    if f'{hpo}_GADOoutput' not in os.listdir(f'{pwd}/{filename}'):
        os.system(f'mkdir {pwd}/{filename}/{hpo}_GADOoutput')
    for i in range(len(df)):
        hpotxt = {}
        case_id = df['Blood ID'][i]
        if f'{case_id}.txt' not in os.listdir(f'{pwd}/{filename}/{hpo}_GADOoutput/') or refresh:
            hpo_id_input = split_hpo_id(df.loc[i,hpo])
            ##所有case记录在hpotxt{}里
            hpotxt[case_id] = hpo_id_input
            with open(f"{pwd}/{filename}/hpotxt/{case_id}_{hpo}_GADO.txt", "w") as file:
                file.truncate(0)
            ...
            cmd1 = f'cd ~/Software/GadoCommandline-1.0.1 && java -jar GADO.jar \
             ...
             --caseHpo {pwd}/{filename}/hpotxt/{case_id}_{hpo}_GADO.txt \
            ...
            os.system(cmd1)
            os.system(cmd2)
            os.system(f'rm -f {pwd}/{filename}/{hpo}_GADOoutput/samples.txt')
        else:
            print(f'{case_id} GADO already finish')
...
def phen2gene_rank(df, pwd, filename, hpo='hpo_id', profile='candidategene', refresh=False):
    ...
    if 'log' not in os.listdir(f'{pwd}/{filename}'):
        os.system(f'mkdir {pwd}/{filename}/log')
    log_phen2gene = pd.DataFrame(columns=['id','run','cmd'])
    if 'phen2geneoutput_nolist' not in os.listdir(f'{pwd}/{filename}'):
        os.system(f'mkdir {pwd}/{filename}/phen2geneoutput_nolist')
    if 'phen2geneoutput' not in os.listdir(f'{pwd}/{filename}'):
        os.system(f'mkdir {pwd}/{filename}/phen2geneoutput')
    for i in range(len(df)):
        case_id = df['Blood ID'][i]
        log_phen2gene.loc[i,'id']=case_id
        hpo_id_input = split_hpo_id(df.loc[i,hpo])
        ## 没有有基因列表
        temp = ''
        ...
        if profile == 'no_gene_list':
            if refresh or (f'{case_id}-phen2gene-nolist-output.txt' not in os.listdir(
                ...
                print(command_phen2gene)
                log_phen2gene.loc[i,'run']=subprocess.getoutput(command_phen2gene)
                log_phen2gene.loc[i, 'cmd'] = command_phen2gene
                print('run phen2gene')
        else:
            if refresh or (f'{case_id}-phen2gene-{profile}-output.txt' not in os.listdir(
                    f'{pwd}/{filename}/phen2geneoutput/')):
                print(f'{case_id} {profile}')
                ...
                print(command_phen2gene_intersect)
                log_phen2gene.loc[i,'run']=subprocess.getoutput(command_phen2gene_intersect)
                log_phen2gene.loc[i, 'cmd'] = command_phen2gene_intersect
                print('run phen2gene')
        log_phen2gene.to_csv(f'{pwd}/{filename}/log/phen2gene_log.csv')
        ## 有基因列表
...
def Phenogenius_rank(df, pwd, filename, hpo, refresh):
    ...
    for i in range(len(df)):
        ...
        if f'{case_id}.tsv' not in os.listdir(f'{pwd}/{filename}/{outputdir}') or refresh:
            if 'HP:' in str(df.loc[i, f'{hpo}_PhenoGenius']):
                hpo_reset = f'{hpo}_PhenoGenius'
                hpo_id_input = split_hpo_id(df.loc[i, hpo_reset])
            else:
                hpo_id_input = split_hpo_id(df.loc[i, hpo])
            hpo_id_list_str = ''
...
def phenolyzer(df, pwd, filename, hpo, refresh=False, refresh_hpo_txt=False):
    ...
    for i in tqdm(range(len(df))):
        ...
        if (f'{case_id}_{hpo}' not in ''.join(os.listdir(f'{pwd}/{filename}/phenolyzeroutput/'))) or refresh:
            cmd_phenolyzer = f'cd ~/Software/phenolyzer && perl disease_annotation.pl {hpo_txt_dir} -file -prediction -phenotype -logistic -out {pwd}/{filename}/phenolyzeroutput/{case_id}_{hpo} -addon DB_DISGENET_GENE_DISEASE_SCORE,DB_GAD_GENE_DISEASE_SCORE -addon_weight 0.25 && mv {pwd}/{filename}/phenolyzeroutput/{case_id}_{hpo}.final_gene_list {pwd}/{filename}/phenolyzeroutput/save_{case_id}.final_gene_list && cd {pwd}/{filename}/phenolyzeroutput && rm -f *{case_id}_{hpo}* && mv {pwd}/{filename}/phenolyzeroutput/save_{case_id}.final_gene_list {pwd}/{filename}/phenolyzeroutput/{case_id}_{hpo}.final_gene_list && exit'
    ...
    if mode == 'intersect':
        for i in range(len(df)):
            ...
            if (f'{case_id}_{hpo}_{filter}_phrank_rank.tsv' not in os.listdir(
                ...
                phrank_gene_ranking.to_csv(f'{pwd}/{filename}/phrankoutput/{case_id}_{hpo}_{filter}_phrank_rank.tsv',
                                           sep='\t')
        ...
        if mode == 'OMIM':
            for i in range(len(df)):
                ...
                if (f'{case_id}_{hpo}_nogenelist_phrank_rank.tsv' not in os.listdir(
                        f'{pwd}/{filename}/phrankoutput')) or refresh:
                    hpo_id_input = split_hpo_id(df.loc[i, hpo])
                    patient_genes_df = pd.read_csv(f'./omim-non-empty.txt', header=None)
            ...
            if hpo != 'Weight':
                ...
                if all_hpo_plus_weight:
                    ...
                    if only_weight_hpo_weight:
                        ...
                        pheno_result = pd.DataFrame(pheno_result)
                        pheno_result.to_csv(
                            f'{pwd}/{filename}/phenoaptoutput/{case_id}_only_{hpo}_hpo_weight_phenoapt_rank.tsv',
                            sep='\t')
                    else:
                        ##只有重点HPO，没有有加权
                        ...
                        pheno_result = pd.DataFrame(pheno_result)
                        pheno_result.to_csv(
                            f'{pwd}/{filename}/phenoaptoutput/{case_id}_only_{hpo}_hpo_no_weight_phenoapt_rank.tsv',
                            sep='\t')
...
def collect_variants(pwd, df, filename='scoliosis_gVCF_from_zs_updating'):
    ##TODO: collect variants from manual filtered TSV
    ...
    for i in tqdm(range(len(df))):
        ...
        file = open(f"{pwd}/{filename}/collected_variants.tsv", "a")
...
def readobo(df, hpo, df_organ, save):
    ...
    for j in tqdm(range(len(df))):
        if str(df.loc[j, f'{hpo}_organ_system']) == 'nan':
            try:
                groupname = []
                hpo_id_input = split_hpo_id(df.loc[j, hpo])
...
def cnv_FA(pwd, filename, df, refresh):
    ...
    for i in tqdm(range(len(df))):
        ...
        if f'{sequence_id}.cnvnator' not in os.listdir(f'{pwd}/{filename}/cnv_function_annotation/'):
            ...
            if f'{sequence_id}.cnv.reg_disruption' not in os.listdir(
                    f'{pwd}/{filename}/cnv_function_annotation/') or refresh:
            ...
            if f'{sequence_id}_standard.bed' not in os.listdir(f'{pwd}/{filename}/cnv_function_annotation/') or refresh:
                ...
                cnvbed.to_csv(f'{pwd}/{filename}/cnv_function_annotation/{sequence_id}_standard.bed', sep='\t',
                              index=False)
    print('cnvFA done')
    return True


def subgroup(df, tools_len, pwd, filename):
    result_file_weight = f'Weight_{tools_len}_rr_len_{len(df)}'
    result_annot = pd.read_csv(f'{result_file_weight}.tsv', sep='\t')

    for k in tqdm(range(len(result_annot))):
        noi_wei = result_annot.loc[k, 'all_hpo_plus_weight_phenoapt_rank']
        n_noi_wei = result_annot.loc[k, 'only_weight_hpo_weight_phenoapt_rank']
        n_noi_n_wei = result_annot.loc[k, 'only_weight_hpo_no_weight_phenoapt_rank']
        noi_n_wei = result_annot.loc[k, 'phenoapt_rank']
        noi = None
        wei = None
        cliInd = None
        concl = None
        if df.loc[k, 'hpo_id'] == df.loc[k, 'Weight']:
            ...
            if (noi is not None) and (wei is not None):
                ...
                if (cliInd, noi, wei) == (0, 1, 0) or (cliInd, noi, wei) == (0, 1, 1) or (cliInd, noi, wei) == (
                1, 1, 1): concl = 4
    ...
    analysis_result = result_annot[result_annot['conclusion'].notna()]
    result_annot.to_csv(f'{result_file_weight}_full_subgrouped.tsv', sep='\t', index=False)
    analysis_result.to_csv(f'{result_file_weight}_classified_subgrouped.tsv', sep='\t', index=False)
...
def compare_df_for_R(df, tools, pwd, mode, dMRR_heatmap=False):
    ...
    for k in range(len(Weight_result)):
        for j in column:
            if j == 'phenoapt_rank':
                combine_result.loc[k + len(hpo_id_result), j] = Weight_result.loc[
                    k, 'only_weight_hpo_no_weight_phenoapt_rank']
            if j == 'phenoapt_intersect_rank':
                combine_result.loc[k + len(hpo_id_result), j] = Weight_result.loc[
                    k, 'only_weight_hpo_no_weight_phenoapt_intersect_rank']
...
def df0_phenoapt_4_strategy_rr(df, tools, pwd):
    ...
    result_strategy = ['only_weight_hpo_no_weight_phenoapt_rank', 'only_weight_hpo_weight_phenoapt_rank',
                       'all_hpo_plus_weight_phenoapt_rank', 'phenoapt_rank']
...
def dMRR_table(tools, combine_result):
    ...
    for group in [1, 2, 3, 4]:
        for n in range(len(tools)):
            ...
            dMRR_table.loc[n, f'Group{group}'] = subset_hpo_id[f'{tools[n]}_rank'].mean() - subset_weight[
                f'{tools[n]}_rank'].mean()
    dMRR_table.to_csv(f'dMRR_{len(combine_result)}.tsv', sep='\t', index=False)
    ##做热图用的dMRR数据，列名固定，需要调整的话，得在R中同步修改


def searchTF_IDF_for_R(df, tools_len, pwd, filename):
    intrisic_weight_df = pd.read_csv(f'{pwd}/weight.csv')
    intrisic_weight_df = intrisic_weight_df.set_index('hpo_id')
    pus_hpo_df = pd.read_csv(f'Weight_{tools_len}_rr_len_{len(df)}_classified_subgrouped.tsv', sep='\t')
...
def searchTF_IDF_for_correlation(tools, pwd, filename):
    intrisic_weight_df = pd.read_csv(f'{pwd}/weight.csv')
    intrisic_weight_df = intrisic_weight_df.set_index('hpo_id')
    pus_hpo_df = read_xlsx(f'{pwd}/{filename}/存档/【统计】hpo_id_13_rank_with_vcf_len_50.xlsx', 'rank')
    pus_hpo_tf_idf = pd.DataFrame(
        columns=['case_id', 'hpo_id_input', 'tools', 'rank', 'tf_idf_sum', 'hpo_gene_anno_number_sum', 'HPO_n',
                 'hpo_id_organ_system_number'])
    m = 0
    for k in tqdm(range(len(pus_hpo_df))):
        ...
        for tool in tools:
            ...
            pus_hpo_tf_idf.loc[m, 'rank'] = pus_hpo_df.loc[k, f'{tool}_rank']
    ...
    pus_hpo_tf_idf.to_csv('HPO_anno_TF_IDF_rank_50.tsv', sep='\t', index=False)
