...
def file_prepare_multiple_source(cohort_df, pwd, filename, intersect, REVEL_thresh=[], CADD_thresh=[], refresh=False):
    ...
    if intersect:
        genertate_intersect_vcf_and_REVEL_Anno_multiple_source_tsv_vcf(cohort_df, pwd, filename, intersect,
                                                                       refresh=refresh)
            ...
            try:
                ...
                if len(CADD_thresh) != 0:
                    for thresh in CADD_thresh:
                        ...
                        vcf_filtered_by_reca(case_id, 'CADD_PHRED', thresh, annt_vcf_filedir, thresh_filter_filedir,
                                             refresh)
                        ## 先把filter的结果和没有过滤的注释后intersect_vcf都存在intersect_vcf_revel;CADD_PHRED
...
def idfile_from_annot(id_name, pwd, filename, case_id, input_vcfdir, profile='candidategene_hgncid', refresh=False):
    ...
    if f'{case_id}.txt' not in os.listdir(f'{pwd}/{filename}/{profile}') or refresh:
        ...
        print(cmd1)
        os.system(cmd1)
        list = load_txt_list(f'{pwd}/{filename}/{profile}/{case_id}.txt')
        df_of_list = pd.DataFrame(list, columns=['0'])
        df_of_list.to_csv(f'{pwd}/{filename}/{profile}/{case_id}.txt', sep='\n', header=False, index=False)
        os.system(f'cd {pwd}/{filename}/{profile} && find . -type f -empty -print -delete')
        print('ok')
        ##重新储存了filter后的新ensembleid
...
def dir_of_Filtered_VCF_MUltiple_source(pwd, filename, case_id, filetype='.vcf'):
    ...
    filedir = 'no Filtered VCF or sVCF anywhere'
    source_mode = 'no Filtered VCF or sVCF anywhere'
...
def generatevcf_Filtered_TSV_intersect_Filtered_VCF(df, pwd, filename, refresh=False):  ##测试记得refresh！！有5个bug case需要这个
    transform_log = pd.DataFrame(
        columns=['case_id', 'tsv_source_mode', 'tsv_file_address', 'vcf_source_mode', 'vcf_file_address', 'bgzip_tabix',
                 'intersect'])
    if 'log' not in os.listdir(f'{pwd}/{filename}'):
        os.system(f'mkdir {pwd}/{filename}/log')
    ...
    for k in tqdm(range(len(df))):
        ...
        print(multi_vcf_filedir)
        if multi_vcf_filedir == 'no Filtered VCF or sVCF anywhere' or refresh:
            os.system(f'rm -f {multi_vcf_filedir}')
            gzdir = get_case_id_file(case_id, 'manual', pwd, filename, filetype='.flt.vcf.gz')
            if gzdir != 0:
                os.system(f'gzip -d {gzdir}')  ##补漏，可能是下载了没有解压，但文件夹里有其他vcf.gz ;此前处理过的文件，不需要再解压一遍
                multi_vcf_filedir, vcf_source_mode = dir_of_Filtered_VCF_MUltiple_source(pwd, filename, case_id)
                ##原地重新解压s.vcf.gz再压缩成 .b.s.vcf.gz
            else:
                gzdir = get_case_id_file(case_id, 'manual', pwd, filename, filetype='.s.vcf.gz')
                if gzdir != 0:
        ...
        if f'{case_id}.vcf' not in os.listdir(f'{pwd}/{filename}/intersect_vcf') or refresh:
            ...
            for file in [f'{pwd}/{filename}/dropbox2vcf/{case_id}.vcf', multi_vcf_filedir]:
                cmd0 = f'bgzip -c -f {file} > {file[:-4]}.b.vcf.gz && bcftools sort {file[:-4]}.b.vcf.gz -o {file[:-4]}.b.vcf.gz -Ou -T {pwd}/{filename}/temp && tabix -p vcf {file[:-4]}.b.vcf.gz'
                print(cmd0)
                try:
                    os.system(cmd0)
                except Exception as e:
            ...
            ##取与Filtered VCF产生的交集
            cmd = f'bcftools isec -n=2 -w1 {multi_vcf_filedir[:-4]}.b.vcf.gz {pwd}/{filename}/dropbox2vcf/{case_id}.b.vcf.gz > {pwd}/{filename}/intersect_vcf/{case_id}.vcf'
            print(cmd)
            try:
                os.system(cmd)
            except Exception as e:
        ...
        # cmdunzip = f'cd {pwd}/{filename}/intersect_vcf && bgzip -d {case_id}.vcf.gz'
        # print(cmdunzip)
        # os.system(cmdunzip)
...
def genertate_intersect_vcf_and_REVEL_Anno_multiple_source_tsv_vcf(df, pwd, filename, intersect, refresh=False):
    ##df = df[2:].reset_index(drop=True)
